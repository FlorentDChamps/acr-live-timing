...
```
src/
  Decode/    protocol decoding: UE packet/bunch parser, FRepLayout reader driven
             by the embedded property layouts, structural replication decoder
             (participants, player states, race states, sectors, results, game
             state), stage/FSM token classification, weather
  Content/   embedded data: content catalog (stage/car names, nation flags) and
             replicated property layouts of the game classes, both generated
             by tools/
...
tools/
  extract-acr-content.py    content catalog from the game's data-table exports
  extract-acr-replayout.py  property layouts from a UE4SS object dump
```
